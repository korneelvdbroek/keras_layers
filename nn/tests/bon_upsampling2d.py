...
class TestQuadraticConv2D(unittest.TestCase):
  def test_equivalence_to_conv2d_transpose(self):
    ...
    inputs = tf.random.uniform(shape=[1, 10, 23, 1], minval=-10., maxval=10., dtype=tf.float32)

    kernel = tf.random.uniform(shape=[6, 16, 1, 1], minval=-10., maxval=10., dtype=tf.float32)
    ...
    for stride_width in range(1, 20):
      ...
      self.assertGreaterEqual(1e-3, tf.reduce_max(tf.abs(outputs - answer)), "Should be zero")
